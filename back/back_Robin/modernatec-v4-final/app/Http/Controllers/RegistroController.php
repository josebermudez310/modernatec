...
class RegistroController extends Controller
{
    ...
    public function export(Request $request)
    {
        ...
        /**mapeamos con el rol que va a ingresar  */
        if ($rol_map == 2)
        /**solo el rol 2 (registros) puede descargar  los registros de los usuarios que entraron */
        {
    ...
    public function UltimoRegistro(Request $request)
    {
        ...
        if ($validator->fails()) // en caso de que lo que esta validando el request falla devolvemos el siguente mensaje
        {
            ...
            /**mapeamos con el rol que va a ingresar  */
            if ($rol_map == 3)
            /**solo el rol 3 (seguridad)pude obtener el ultimo registro del usuario ingresado  */
            {
                /**consulta eloquent para traer el ultimo contrato del usuario  */
                $registros =  DB::table('users')
                ->join('registros', 'users.id', '=', 'registros.numero_identificacion')
                ->select('registros.*','users.name','users.numero_identificacion' )
                ->where('users.id', [$request->id])
                ->orderByRaw("concat(fecha, '', hora_ingreso) DESC")
                ->first();
