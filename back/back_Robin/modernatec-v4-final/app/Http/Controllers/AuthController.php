...
class AuthController extends Controller
{
    ...
    public function Registro_Invitado(Request $request, Faker $faker)
    {
        ...
        $validator =  Validator::make(
            ...
            [
                ...
                'numero_identificacion' => 'required|min:5|unique:users',
    ...
    public function login(Request $request, Faker $faker)
    {
        ...
        } else //en caso de que el usuario no tenga el correo confirmado se llevaran a cabo las siguientes acciones
        {
            ...
            return response()->json(['message' => 'Su correo no se encuentra confirmado, por favor confirme el correo que se le ha enviado ']);
        }
    }
    ...
    {
        $validator =  Validator::make(
            $request->all(), // la variable validator va a contener todo lo que nos envia por el request
            [
                // validamos los campos que esta resibiendo el request para registrar a un nuevo usuario
                // los campos como los estamos llamando en el request se llaman igual en la base de datos
                'email' => 'required',
     ...
     * Creacion de ruta para actualizacion de contraseña 
     */
    /**esta ruta va a actualizar la contraseña mientras al usuario no se le halla olvidado la contraseña anterior */
    public function password_update(Request $request)
    {
        ...
        if ($rol_map){
            $credentials = request(['email', 'password']);/**asignamos a la variable credentials el request del email y el password y con este podremos validar que la contraseña actual que estan ingresando sea correcta  */

            if (! $token = auth()->attempt($credentials))/**validamos la contraeña actual en caso de que sea incorrecta mostramos el siguiente mensaje  */
             {
                return response()->json(['error' => 'Contraeña no coinside'], 401);/**mensaje de que la contraseña no coinside con un error de servidor 401 */
            }else{
                DB::table('users') /**le actualizamos la contraseña y la incriptamos   */
                ->where('email', $request->email)
                ->update(['password' => bcrypt($request->password_update)]);
            return response()->json([ /**respondemos con el siguiente mensaje  */
                'message' => "se cambio la contraseña"
            ]);
            }
